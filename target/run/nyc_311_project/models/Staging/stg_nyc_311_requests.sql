  ...
  create or replace view `depi-graduation-project-489604`.`nyc_311_raw_data`.`stg_nyc_311_requests`
  OPTIONS()
  as  -- طبقة الـ Staging يفضل أن تكون View لتوفير المساحة

WITH raw_data AS (
    SELECT * FROM `depi-graduation-project-489604`.`nyc_311_raw_data`.`raw_service_requests` -- هنا dbt سيفهم أنه يقرأ من الجدول الذي قمنا بحمايته
)
